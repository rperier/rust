...
pub fn for_each_expr<'tcx>(
    ...
    f: impl FnMut(&'tcx hir::Expr<'tcx>) -> bool,
) {
    struct V<'tcx, F> {
        _tcx: TyCtxt<'tcx>,
        f: F,
    ...
    impl<'tcx, F: FnMut(&'tcx hir::Expr<'tcx>) -> bool> Visitor<'tcx> for V<'tcx, F> {
    ...
    fn check_fn(
        ...
        if safety.is_safe() && cx.effective_visibilities.is_exported(def_id) {
            ...
            if !raw_ptrs_and_int_params.is_empty() {
                ...
                for_each_expr(cx.tcx, body.value, |e: &'tcx hir::Expr<'tcx>| {
                    match e.kind {
                        ...
                        hir::ExprKind::Unary(hir::UnOp::Deref, _ptr) => {}
                        _ => {}
                    }
                    true
                });
